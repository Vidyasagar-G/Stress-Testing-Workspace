...
shift_vector = [2.0, -1.5, 1.0, 0.5, -0.5]  # For the first few latent dims
...
for start_idx in range(0, len(returns_matrix) - window_size + 1, step_size):
    ...
    # === STEP 1: AUTOENCODER ===
    ae = AutoencoderReducer(window_returns, encoding_dim=5, epochs=100, scale_factor=100, batch_size=32)
    ...
    print(f"time in train + reconstruct: {start_idx+1} -> ", time.time() - start_time)
    
    # saving loss plot for some windows
    if start_idx%20 == 0:
        ae.plot_reconstruction_loss(save_path=f"loss_plot_{window_dates[0]}_{window_dates[1]}.png")

    # === STEP 2: STRESS SCENARIO ===
    scenario_gen = AEScenarioGenerator(latent)

    for stress_mode in ["single", "multi"]:
    ...
    print(f"time in completing 1 loop: {start_idx+1} -> ", time.time() - start_time)

# === EXPORT RESULTS === #
df_single = pd.json_normalize(results_single)
df_multi = pd.json_normalize(results_multi)
df_single.to_csv("rolling_ae_stress_with_deltas_single.csv", index=False)
df_multi.to_csv("rolling_ae_stress_with_deltas_multi.csv", index=False)
